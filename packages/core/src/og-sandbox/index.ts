...
/**
 * Sepolia testnet provider's PROXY_DOMAIN. Inbound HTTP to a sandbox arrives
...
/**
 * Sepolia testnet provider's TEE signer (v3). Stable per signer-version; the
...
/**
 * Sepolia testnet provider's wallet address. Returned by `/api/providers`.
 */
export const SANDBOX_PROVIDER_GALILEO = '0xB831371eb2703305f1d9F8542163633D0675CEd7' as const

/**
 * Sepolia testnet provider's HTTP base URL. The `/info`, `/api/providers`,
...
/**
 * Sepolia testnet sandbox runtime billing rate, in Mantle/hour. Surfaced in the
 * init wizard's cost summary and by `nebula topup --sandbox` for the runway
 * estimate `balance / rate`. Sepolia testnet Mantle is faucet-funded; the rate
 * is real but the dollar cost is $0.
 */
export const SANDBOX_BURN_RATE_OG_PER_HOUR = 0.09

/**
 * Default initial deposit on the Sepolia SandboxSettlement contract when

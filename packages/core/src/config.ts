...
export interface INFTRef {
  ...
  contract: string
  /** Token id minted to the owner at `nebula init`. */
  tokenId: string
 ...
 * `nebula` (chat), `nebula topup`, and `nebula restore` need to talk to the
...
export interface NebulaConfig {
  ...
  /**
   * Phase 7: the agent's `<label>.nebula.0g` subname (without the suffix).
   * Recorded by `nebula init` so the chat loop can auto-publish the agent's
   ...
   * `skills.manage` and persisted to ~/.nebula/config.ts.
   */
  skills?: {
    disabled?: string[]
  }
   ...
   * concatenated under a `# Operator instructions` header AFTER nebula's
   ...
   * allowlist (agentDir + workspaceRoot + /tmp/nebula-* + /var/folders).
   *
  ...
  sandbox?: {
     ...
     * Hermes default is 5120 (5GB); leaving unset is nebula's default so the

...
export async function runRestore(opts: { ref: string; cwd?: string }): Promise<void> {
  ...
  if (version === 1) {
    note(
      'Detected legacy v1 (passphrase) keystore. After restore, run `nebula migrate-keystore` to upgrade to v2 (operator-wallet-encrypted).',
      'legacy keystore',
    )
  ...
  await writeConfigTs(configPath, cfg, {
    header: '// Regenerated by `nebula restore`. Edit freely; type-safe.',
    subname: null,
  })
  ...
  outro(
    [
      ...
      '',
      'Next: `nebula` to chat, or `nebula topup --compute 5` if ledger is dry.',
      version === 1
        ? 'Then: `nebula migrate-keystore` to upgrade to v2 (drops the passphrase).'
        : '',
    ]
      .filter(Boolean)
      .join('\n'),

...
export function makeBrowserVision(
  ...
  return {
    name: 'browser.vision',
    description:
      "Capture the current page as a screenshot and send it to the configured vision model with a prompt. Returns the model's reply. Routes to the configured multimodal vision provider (qwen3-vl-30b by default).",
    shouldDefer: true,
    ...
    handler: async args => {
      if (!deps.visionInfer) {
        return {
          ok: false,
          error:
            'vision provider not configured. Set `vision.provider` in ~/.nebula/config.ts to a multimodal provider.',
        }
      }

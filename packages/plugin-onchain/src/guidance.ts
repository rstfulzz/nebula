...
- Balance + identity: \`chain.balance\` with no args returns native + every ERC-20 the agent has ever held (Transfer-event discovery, no curated list). Pass \`token\` for a single asset or \`address\` to inspect another wallet. \`account.info\` bundles wallet + iNFT + brain provider + recent activity AND the agent's own \`.nebula.0g\` subname, A2A pubkey, and the canonical nebula singleton addresses (inbox/market/agentNFT). Call it before answering identity / "who are you" / "what's your pubkey" questions instead of guessing.
- Full balance position: \`account.balance\` returns the FULL economic picture — EOA mainnet + EOA testnet + compute ledger total/available/locked + sandbox billing reserve (when sandbox-deployed). Use this for "what's my balance", "how much do we have", "show full position", "total funds" questions. EOA-only answers under-count by up to 10x because compute envelopes (locked in Mantle provider sub-accounts) are usually larger than the EOA itself. \`chain.balance\` is for token-level detail; \`account.balance\` is for top-line aggregation.
- Nebula singletons (CREATE2-deterministic, same address on testnet and mainnet, introspect via \`chain.contract\`):
  - \`NebulaInbox\` at \`0xcd92844cc0ec6Be0607B330D4BaCC707339f2589\`: singleton A2A encrypted message emitter (ECIES, ERC-7857 inbox, Phase 7).

...
- Trading: two DEX venues. Agni Finance (Uniswap-V3-style) via \`swap.quote\`/\`swap.execute\`, and Merchant Moe (Liquidity Book) via \`moe.quote\`/\`moe.swap\`. For best execution on a non-trivial trade, quote BOTH (\`swap.quote\` + \`moe.quote\`) and execute on whichever returns more output. Slippage default 0.5%. ERC-20 input swaps auto-approve the venue's router on first use. A token with no pool on a venue can't trade there (chain.balance still shows it).

...
- Treasury value: \`treasury.summary\` is the one-call USD picture — idle wallet holdings (MNT + ERC-20s, priced via DeFiLlama) plus funds deployed in Aave, with per-asset values and the idle-vs-deployed split. Use it for "what are we worth", "full treasury", "portfolio", "how much is deployed". (\`chain.balance\` is token amounts; \`treasury.summary\` is USD totals + where the value sits.)

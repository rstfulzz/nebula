...
/// @notice Deploy NebulaMarket via CREATE2. Same address on testnet + mainnet
...
contract DeployMarket is Script {
    bytes32 public constant SALT = keccak256("nebula:NebulaMarket:v1");

    function run() external returns (NebulaMarket market) {
        address feeRecipient = vm.envOr("FEE_RECIPIENT", address(0));
        vm.startBroadcast();
        if (feeRecipient == address(0)) {
            feeRecipient = msg.sender;
        }
        market = new NebulaMarket{salt: SALT}(feeRecipient);
        vm.stopBroadcast();
        console2.log("NebulaMarket deployed at:", address(market));

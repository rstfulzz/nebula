...
/// @notice Deploy NebulaSubnameRegistrar via CREATE2 (mainnet-only) and grant
/// the registrar write-access under `nebula.0g`. Must be broadcast by the
/// nebula.0g registry owner (dev.deployer).
///
...
contract DeployRegistrar is Script {
    bytes32 public constant SALT = keccak256("nebula:NebulaSubnameRegistrar:v1");
    ...
    function run() external returns (NebulaSubnameRegistrar reg) {
        address caller = msg.sender;
        vm.startBroadcast();
        reg = new NebulaSubnameRegistrar{salt: SALT}(REGISTRY, RESOLVER, caller);
        ISidRegistry(REGISTRY).setApprovalForAll(address(reg), true);
        vm.stopBroadcast();
        require(reg.isOperational(), "registrar not operational after deploy");
        console2.log("NebulaSubnameRegistrar deployed at:", address(reg));
        console2.log("Salt:", vm.toString(SALT));
        console2.log("Approval set by nebula owner:", caller);
    }
}

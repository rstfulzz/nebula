...
/// @title NebulaInbox
/// @notice Singleton A2A message emitter for nebula agents on Mantle Chain.
...
/// the ECIES payload, or fetch the Mantle Storage blob via `dataHash` if set.
///
...
contract NebulaInbox {
    /// @notice Hard cap on inline payload bytes. Nebula's spillover threshold
    ...
    /// route through Mantle Storage (which carries its own write fee).
    ...
    /// @param dataHash Mantle Storage pointer (over threshold or files), or zero.
    event Message(
    ...
    /// @param dataHash Mantle Storage hash, or `bytes32(0)` if payload is inline.

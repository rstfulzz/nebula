...
pub enum IdentityError {
    AgentAddressAlreadyRegistered = 1,
    NotAuthorized = 2,
    UnknownAgent = 3,
}
...
impl IdentityRegistry {
    ...
    pub fn register(&mut self, card_uri: String, agent_address: Address) -> u64 {
        if self.agent_id_by_address.get_or_default(&agent_address) != 0 {
            self.env().revert(IdentityError::AgentAddressAlreadyRegistered);
        }
    ...
    pub fn resolve(&self, agent_id: u64) -> (Address, Address, String) {
        ...
        let agent_address = self
            ...
            .get(&agent_id)
            .flatten()
            .unwrap_or_revert_with(&self.env(), IdentityError::UnknownAgent);
    ...
    fn assert_owner(&self, agent_id: u64) -> Address {
        ...
        if owner != self.env().caller() {
            self.env().revert(IdentityError::NotAuthorized);
        }
        owner
    }
}
...
mod tests {
    ...
    fn duplicate_agent_address_reverts() {
        ...
        assert_eq!(
            ...
            IdentityError::AgentAddressAlreadyRegistered.into()
        );
    }
    ...
    fn only_owner_updates_card() {
        ...
        assert_eq!(
            ...
            IdentityError::NotAuthorized.into()
        );
    }
}
